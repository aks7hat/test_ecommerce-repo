  ...
  useEffect(()=>{
    ...
    allOrders?.map(order=>{
      ...
      displayOrders.push({
        ...
        status:(order?.orderStatus === 'PENDING' || order?.orderStatus === 'IN_PROGRESS' || order?.orderStatus === 'SHIPPED' || order?.orderStatus === 'CREATED')? 'ACTIVE': order?.orderStatus === 'DELIVERED' ? 'COMPLETED':order?.orderStatus,
