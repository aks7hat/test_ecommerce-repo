    ...
    <div className='p-4'>
                    ...
                    cartItems?.map((item,index)=>{
                        return (
                            <tr className='p-4 bg-white border-b'>
                                <td>
                                    <div className='flex p-4'>
                                       ...
                                       <div className='flex flex-col text-sm px-2 text-gray-600'>
                                        <p><b>{item?.name || 'Name'}</b></p>
                                        <p><b>Size - </b> {item?.variant?.size}</p>
                                        <p><b>Color - </b> {item?.variant?.color}</p>
                                       </div>         
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                <NumberInput max={4} quantity={item?.quantity} onChangeQuantity={(value)=> onChangeQuantity(value, item?.productId, item?.variant?.id)}/>
                                </td>
                                ...
                                <td>
                                    <button className='flex justify-center items-center w-10 h-10 bg-red-500 hover:bg-red-600 text-white rounded-full shadow transition-transform duration-300 transform hover:scale-110' onClick={()=> onDeleteProduct(item?.productId, item?.variant?.id)} style={{marginLeft:'60px'}}><DeleteIcon /></button>
                                </td>
                            </tr>
        ...
        </>}
        {
            ...
            <div className='w-full items-center text-center'>
                ...
                <div className='p-4'><Link to={"/products/all"} className='w-full p-2 items-center h-[48px] bg-black border rounded-lg mt-2 text-white hover:bg-gray-800'>Continue Shopping</Link></div>
            </div>
        }
    </div>
    <Modal
